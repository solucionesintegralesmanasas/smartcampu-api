...
const EmailConsumer = require('./jobs/consumers/email.consumer');
  ...
  const gracefulShutdown = async (signal) => {
    ...
    if (server) {
      server.close(async () => {
        ...
        try {
          if (emailConsumer) await emailConsumer.close();
          logger.info('Email consumer closed');
        } catch (err) {
          logger.error('Error closing email consumer', { error: err.message });
        }
        process.exit(0);
      });
    }
